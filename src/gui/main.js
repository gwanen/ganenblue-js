...
const createStatsCallback = (profileId, instance) => (stats) => {
  ...
  if (instance.stats.startTime) {
    ...
    const completed =
      stats.battleCount || stats.completedQuests || stats.raidsCompleted || 0;
    if (diff > 5000) {
...
ipcMain.handle(
  "browser:launch",
  async (event, profileId, browserType = "chromium", deviceSettings = {}) => {
    const instance = getInstance(profileId);

    // Prevent multiple browser instances for the same profile
    if (instance.browser) {
      logger.info(
        `[System] [${profileId}] Browser already active. Reusing instance...`,
      );
    ...
    try {
      ...
      await instance.browser.navigateAndLogin(
...
ipcMain.handle("bot:start", async (event, profileId, settings) => {
  ...
  if (!instance.browser || !instance.browser.page) {
    return {
      success: false,
      message: 'Browser not initialized. Click "Open Browser" first.',
    };
  }

  try {
    logger.info(
      `[System] [${profileId}] Starting automation in ${settings.botMode} mode...`,
    );
...
ipcMain.handle("bot:stop", async (event, profileId) => {
  ...
  if (instance.bot) {
    ...
    if (instance.bot.battle && instance.bot.battle.controller) {
      instance.bot.battle.controller
        .stop()
        .catch((e) => logger.debug("[Cleanup] Controller stop error:", e));
    }
    instance.bot = null;
  }
...
ipcMain.handle("bot:reset-stats", (event, profileId) => {
  ...
  if (instance.bot) {
    ...
    if (typeof instance.bot.battleCount !== "undefined")
...
ipcMain.handle("credentials:save", async (event, profileId, credentials) => {
  try {
    ...
    logger.error(
      `[Error] [Gui] [${profileId}] Credential update failure:`,
      error.message,
    );
    return { success: false, message: error.message };
  }
...
ipcMain.handle("credentials:load", async (event, profileId) => {
  try {
    ...
    logger.error(
      `[Error] [Gui] Failed to load credentials for ${profileId}:`,
      error.message,
    );
    return { success: false, message: error.message };
  }
...
class GuiTransport extends winston.Transport {
  log(info, callback) {
    if (mainWindow && !mainWindow.isDestroyed()) {
      ...
      if (
        info.level === "error" &&
        (info.message.includes("Captcha") || info.message.includes("[Safety]"))
      ) {
        const isLogout =
          info.message.includes("Logged out") ||
          info.message.includes("Session");
        const pid = info.profileId || "sys";
        showNotification(
          isLogout ? "🚨 SESSION EXPIRED 🚨" : "⚠️ ACTION REQUIRED ⚠️",
          isLogout
            ? `[${pid}] Session expired. Please log in again.`
            : `[${pid}] Captcha Detected - Manual intervention required.`,
          true,
        );
      }
    }
